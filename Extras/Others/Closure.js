...
// In JavaScript, every time a new function is defined, a new lexical environment is created. A lexical environment consists of all the variables and functions
//that were in scope at the time the function was defined. This means that a function can access variables and functions that are in its own lexical environment,
//as well as any lexical environments that contain it. A closure is a function that has access to variables from its outer (enclosing) lexical environment, even
//after that outer function has returned. This is possible because the closure "closes over" those variables and keeps them alive, so they can still be accessed by
//the closure even though the outer function has completed execution.
